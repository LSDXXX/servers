...
func (m Migrator) HasColumn(value interface{}, field string) bool {
	...
	return m.RunWithValue(value, func(stmt *gorm.Statement) error {
		return m.DB.Raw("SELECT COUNT(*) FROM USER_TAB_COLUMNS WHERE TABLE_NAME = ? AND COLUMN_NAME = ?",
			stmt.Table, field).Row().Scan(&count)
	}) == nil && count > 0
}

// CreateConstraint description
// @receiver m
// @param value
// @param name
// @return error
...
// RenameIndex description https://docs.oracle.com/database/121/SPATL/alter-index-rename.htm
// @receiver m
// @param value
// @param oldName
// @param newName
...
func (m Migrator) TryRemoveOnUpdate(value interface{}) error {
	return m.RunWithValue(value, func(stmt *gorm.Statement) error {
		for _, rel := range stmt.Schema.Relationships.Relations {
			...
			if constraint != nil {
				rel.Field.TagSettings["CONSTRAINT"] =
					strings.ReplaceAll(rel.Field.TagSettings["CONSTRAINT"], fmt.Sprintf("ON UPDATE %s", constraint.OnUpdate), "")
			}
		}
		return nil
	})
...
// TryQuotifyReservedWords description
// @receiver m
// @param values
// @return error
